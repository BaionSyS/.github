...
- [`baion-std`](https://github.com/BaionSyS/baion-std) — seven languages (C, C++, Rust, Go, D, Haskell, OCaml) producing byte-identical canonical JSON + SHA-256, with a one-command cross-lineage verifier, re-run in CI on every push to `main` and every pull request — checking byte-identical agreement across all seven lineages over the tested corpus and differential probes (tested agreement on the covered inputs, not a proof of canonicalization correctness against an independent oracle). The trust discipline underneath our multi-component systems.
